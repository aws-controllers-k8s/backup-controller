...
func (r *resource) SetIdentifiers(identifier *ackv1alpha1.AWSIdentifiers) error {
	...
	r.ko.Status.SelectionID = &identifier.NameOrID

	f0, f0ok := identifier.AdditionalKeys["backupPlanID"]
	if f0ok {
		r.ko.Spec.BackupPlanID = aws.String(f0)
	}
...
func (r *resource) PopulateResourceFromAnnotation(fields map[string]string) error {
	primaryKey, ok := fields["selectionID"]
	if !ok {
		return ackerrors.NewTerminalError(fmt.Errorf("required field missing: selectionID"))
	}
	r.ko.Status.SelectionID = &primaryKey
	f0, ok := fields["backupPlanID"]
	if !ok {
		return ackerrors.NewTerminalError(fmt.Errorf("required field missing: backupPlanID"))
	}
	r.ko.Spec.BackupPlanID = &f0
