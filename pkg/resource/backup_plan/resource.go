...
func (r *resource) SetIdentifiers(identifier *ackv1alpha1.AWSIdentifiers) error {
	...
	r.ko.Status.BackupPlanID = &identifier.NameOrID

	f2, f2ok := identifier.AdditionalKeys["versionID"]
	if f2ok {
		r.ko.Status.VersionID = aws.String(f2)
	}
...
func (r *resource) PopulateResourceFromAnnotation(fields map[string]string) error {
	primaryKey, ok := fields["backupPlanID"]
	if !ok {
		return ackerrors.NewTerminalError(fmt.Errorf("required field missing: backupPlanID"))
	}
	r.ko.Status.BackupPlanID = &primaryKey

	f2, f2ok := fields["versionID"]
	if f2ok {
		r.ko.Status.VersionID = aws.String(f2)
	}
